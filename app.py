...
#    profile = line_bot_api.get_profile(event.source.user_id)
        ...
        buttons_template = TemplateSendMessage(
            ...
            template=ButtonsTemplate(
                ...
                actions=[
                    ...
                    MessageTemplateAction(
                        label='詳細功能說明',
                        text='詳見小星星粉絲頁http://pcse.pw/83A5Q'
                    ),
                    MessageTemplateAction(
                        label='小星星粉絲頁',
                        text='http://pcse.pw/83A5Q'
                    )
                ]
            )
        )
    ...
    if re.match('^dimg=https://i.imgur.com/(.+).jpg',event.message.text):
        img_url = re.match('^dimg=https://i.imgur.com/(.+).jpg',event.message.text).group(1)
        print(img_url)
    ...
    if event.message.text == '功能統計':
        ...
        line_bot_api.reply_message(event.reply_token,TextSendMessage(text=content))
        return 0
    if event.message.text[0:3] == '查證=':
        res = re.search("查證=(.+)",event.message.text).group(1)
        content = mygopen(res)
        line_bot_api.reply_message(event.reply_token,TextSendMessage(text=content))
        gs_write('B25')
        return 0
    if event.message.text[0:3] == '巴哈=':
